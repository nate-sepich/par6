...
struct LeaderboardSection: View {
    ...
    @State private var selectedPlayer: (userId: String, handle: String)?
    ...
    var body: some View {
        VStack(spacing: 16) {
            ...
            LazyVStack(spacing: 8) {
                ForEach(Array(standings.enumerated()), id: \.element.id) { index, standing in
                    Button(action: {
                        // Don't show stats for current user - they can see their own in Scorecard
                        if !standing.isCurrentUser {
                            selectedPlayer = (userId: standing.userId, handle: standing.handle)
                        }
                    }) {
                        LeaderboardRow(
                            standing: standing,
                    ...
                    .buttonStyle(.plain)
                    .disabled(standing.isCurrentUser)
                }
            }
        }
        .padding(20)
        ...
        .sheet(item: Binding(
            get: { selectedPlayer.map { IdentifiablePlayer(userId: $0.userId, handle: $0.handle) } },
            set: { _ in selectedPlayer = nil }
        )) { player in
            PlayerStatsView(
...
// Helper struct to make the player selection identifiable for sheet presentation
